...
export PYTHONPATH="$(dirname "$(dirname "$(dirname "$REAL_AICO_ENTRY")")")/src:$PYTHONPATH"
...
if [ "$1" = "edit" ]; then
    ...
    python3 -c "import pty, sys, os; pty.spawn([sys.executable, '$REAL_AICO_ENTRY'] + sys.argv[1:])" "$@" | tr -d '\r'
else
    # Standard execution for non-interactive commands
    python3 "$REAL_AICO_ENTRY" "$@"
fi
