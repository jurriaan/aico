...
pub fn count_heuristic(text: &str) -> u32 {
    let mut counter = HeuristicCounter::default();
    let _ = counter.write_str(text);
    counter.count()
}
...
pub fn count_tokens_for_messages(messages: &[&str]) -> u32 {
    ...
    for message in messages {
        let _ = counter.write_str(message);
    }
    counter.count()
}
...
mod tests {
    ...
    fn test_count_heuristic() {
        assert_eq!(count_heuristic("acd"), 1);
    ...
    fn test_count_tokens_for_messages() {
        let messages = vec!["Hello", "World!", "This is a test."];

        assert_eq!(count_tokens_for_messages(&messages), 7);
    }
